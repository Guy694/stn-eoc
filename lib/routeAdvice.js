...
function findDistrict(text, districts, preferredDirection = "from") {
...
export function parseRouteQuestion(message, districts = []) {
  return {
    from: findDistrict(message, districts, "from"),
    to: findDistrict(message, districts, "to")
  };
}

async function fetchDistricts() {
  const rows = await query(`
    SELECT MIN(id) AS id, distname AS name,
           AVG(ST_X(ST_Centroid(ST_GeomFromWKB(ST_AsWKB(geom), 0)))) AS lat,
           AVG(ST_Y(ST_Centroid(ST_GeomFromWKB(ST_AsWKB(geom), 0)))) AS lon
  ...
  return rows.map((row) => ({
    ...
    lat: Number(row.lat),
    lon: Number(row.lon),
    aliases: [row.name, `อำเภอ${row.name}`, `อ.${row.name}`]
  }));
}

async function fetchRouteCorridors() {
  if (!await hasTable("route_corridors")) return [];
  const rows = await query(`
    SELECT corridor_key, corridor_name, district_names, route_hint
    FROM route_corridors
    WHERE is_active = TRUE
    ORDER BY id
  `);
  return rows.map((row) => {
    const names = typeof row.district_names === "string" ? JSON.parse(row.district_names) : row.district_names;
    return {
      key: row.corridor_key,
      name: row.corridor_name,
      districts: Array.isArray(names) ? names : [],
      labels: Array.isArray(names) ? names : [],
      routeHint: row.route_hint || "ตรวจสอบข้อมูลเส้นทางและรายงานภาคสนามก่อนเดินทาง"
    };
  });
}
...
function routeDistricts(from, to, corridors) {
  ...
  return [{
    ...
    routeHint: `เส้นทางนี้เป็นผลคำนวณเบื้องต้น โปรดตรวจรายงานถนนและการยืนยันจากเจ้าหน้าที่ก่อนเดินทาง`
  }];
}

function scoreRoute(route, from, to, floodRisks, trafficReports, allDistricts) {
  const routeDistrictNames = new Set(route.districts);
  const routePoints = route.labels
    .map((name) => allDistricts.find((district) => district.name === name))
    .filter(Boolean);
  const floodMatches = floodRisks.filter((item) => routeDistrictNames.has(item.district));
  const trafficMatches = trafficReports.filter((item) => {
    if (routeDistrictNames.has(item.district)) return true;
...
export async function getRouteAdvice({ message, from: fromInput, to: toInput }) {
  const [districts, corridors] = await Promise.all([fetchDistricts(), fetchRouteCorridors()]);
  const parsed = parseRouteQuestion(message || "", districts);
  const from = typeof fromInput === "string" ? findDistrict(fromInput, districts, "from") : fromInput || parsed.from;
  const to = typeof toInput === "string" ? findDistrict(toInput, districts, "to") : toInput || parsed.to;
  ...
  const routes = routeDistricts(from, to, corridors)
    .map((route) => scoreRoute(route, from, to, floodRisks, trafficReports, districts));
  ...
  return {
    ...
    meta: {
      ...
      database_sources: ["flood_records", "public_incident_reports", "route_corridors", "satun_village_polygon"],
      external_provider: osrmRoutes ? "OSRM" : null,
      generated_at: new Date().toISOString(),
      warning: "ไม่พบรายงานไม่ได้หมายความว่าเส้นทางปลอดภัย ต้องตรวจสอบกับเจ้าหน้าที่ภาคสนาม"
    },
