...
export function resolveThaiIdCallbackUrl(request, callbackPath) {
    ...
    function normalizeCallbackPath(url) {
        ...
        if (url.pathname === '/' || url.pathname === '') {
            ...
            configuredPathWithoutBase === legacyPath ||
            configuredPathWithoutBase.startsWith('/api/auth/')
        ) {
            url.pathname = appCallbackPath;
        }
...
export function getThaiIdConfigError(config, { requireClientSecret = false } = {}) {
    ...
    if (!config.apiKey) missing.push('THAIID_API_KEY หรือ APIKEY');
    ...
    return missing.length ? `ตั้งค่า ThaiID ไม่ครบ: ${missing.join(', ')}` : null;
}
