...
const floodTypes = ["น้ำท่วมขัง", "น้ำป่าไหลหลาก", "น้ำล้นตลิ่ง", "ดินสไลด์จากฝน"];
...
function buildMeetingNotes(cycles, decisions) {
  return cycles.map((cycle, index) => ({
    ...
    agenda: "สรุปสถานการณ์น้ำท่วมประจำวัน, ตรวจสอบข้อมูลทีม, พิจารณาข้อสั่งการ",
...
function buildDailySummaries(cycles, floodReports, shelters, healthFacilities, missions, teamInputs) {
  return cycles.map((cycle) => {
    ...
    return {
      ...
      situation_summary: `ฝนตกหนักต่อเนื่อง ส่งผลให้มีจุดน้ำท่วม ${floodToday.length} จุด ครอบคลุม ${affectedDistricts} อำเภอ ความครบถ้วนข้อมูล ${cycle.data_completeness_score}%`,
