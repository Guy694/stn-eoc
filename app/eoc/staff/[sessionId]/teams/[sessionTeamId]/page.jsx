...
export default function TeamWorkspacePage() {
    ...
    const normalizedTeamCode = normalizeTeamCode(team?.team_code);
    const reportDefinition = getTeamReportDefinition(normalizedTeamCode);
    const reportLink = team && session && normalizedTeamCode ? {
        href: `/eoc/${session.eoc_type}/teams/${normalizedTeamCode}/records?sessionId=${sessionId}`,
        label: reportDefinition?.title || "รายงานผลกลุ่มภารกิจ"
    } : null;
        ...
        <EOCLayout>
            <section className="mx-auto max-w-6xl space-y-5">
                ...
                {loading ? <div className="bg-white p-10 text-center text-sm text-slate-500">กำลังโหลดพื้นที่ทำงาน...</div> : error ? <div className="border border-red-200 bg-red-50 p-5 text-sm font-medium text-red-800">{error}</div> : team && <>
                    ...
                    {tab === "reports" && <div className="bg-white p-5 shadow-sm"><h2 className="text-lg font-black text-slate-900">รายงานผลของกลุ่มภารกิจ</h2><p className="mt-1 text-sm text-slate-600">การบันทึกข้อมูลผูกกับ EOC Session และกลุ่มภารกิจนี้โดยอัตโนมัติ เมื่อปิด Session แล้วยังสามารถจัดทำ แก้ไข ส่ง และตรวจรายงานย้อนหลังได้ตามสิทธิ์</p>{reportLink ? <Link href={reportLink.href} className="mt-5 inline-flex items-center gap-2 bg-cyan-700 px-4 py-3 text-sm font-bold text-white hover:bg-cyan-800"><BarChart3 className="h-4 w-4" />{reportLink.label}</Link> : null}</div>}
