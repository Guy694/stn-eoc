...
export default function AnnouncementDetailPage({ params }) {
  ...
  useEffect(() => {
    fetch(`/stn-eoc/api/public/announcements?id=${encodeURIComponent(id)}&include_old=1`)
      ...
      .catch((error) => console.error('Load announcement detail error:', error))
      .finally(() => setLoading(false));
  }, [id]);

  return <EOCLayout><div className="mx-auto max-w-5xl">
    <Link href="/announcements" className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-900"><ArrowLeft className="h-4 w-4" />กลับหน้าประกาศ</Link>
